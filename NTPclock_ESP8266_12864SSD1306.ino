...
static int scrnoff = 22; 									//屏幕关闭的时间，24小时制，不想关填24
static int scrnon = 6;										//屏幕开启时间，24小时制，不想关填0
...
void oledClockDisplay()
{
	...
	if (isNTPConnected) { u8g2.print("当前时间 (UTC+8)");}
	else { u8g2.print("无网络!");} //如果上次对时失败，则会显示无网络
	
	String currentTime = "";
	if (m != 0)
	{
		if (hours < 12) { m = 1;} else { m = 2;}
	}
	if (hours < 10) { currentTime += 0;}
	currentTime += hours;
	currentTime += ":";
	...
	if (m != 0)
	{
		if (m == 2) { currentTime += "P";} else { currentTime += "A";}
	}
	String currentDay = "";
	currentDay += years;
	currentDay += "/";
	if (months < 10) { currentDay += 0;}
	currentDay += months;
	currentDay += "/";
	if (days < 10) { currentDay += 0;}
	currentDay += days;
